...
;; Package-Version: 20260223.1721
;; Package-Revision: 1fc1d8f2d56e
...
(defconst company-coq-old-path-regexp "\\(?1:[^ \n]+\\)\\(?: \\|\n  \\)\\(?:\"\\(?2:[^\n]+\\)\"\\|\\(?2:[^\n]+\\)\\)")
(defconst company-coq-new-path-regexp (concat "\\(?:i\\| \\)\\(?: \\|\n  \\)" company-coq-old-path-regexp))
...
(defcustom company-coq-prettify-symbols-alist
  '(;; Disabled
...
(defun company-coq-scan-path-specs (line-regex)
  "Read module path specs from current buffer according to LINE-REGEX.
  ...
  (let ((specs nil))
    ...
    (save-match-data
      (while (re-search-forward line-regex nil t)
        (push `(,(match-string-no-properties 1) . ,(match-string-no-properties 2)) specs)))
    (nreverse specs)))
...
Returns a list of pairs of (LOGICAL . PHYSICAL) paths."
  (when loadpath-output
    (with-temp-buffer
      (insert loadpath-output)
      (company-coq-scan-path-specs
       (if (string-prefix-p "Installed / Logical Path / Physical path:" loadpath-output) ;; 9.1+
           company-coq-new-path-regexp
         company-coq-old-path-regexp)))))
...
(defun company-coq-init-modules (&optional force)
  ...
  (interactive '(t))
  (when (or (company-coq-feature-active-p 'modules-backend)
            (company-coq-feature-active-p 'cross-ref))
...
(defun company-coq--strip-dune-build-dir (path)
  "Remove dune build directory part of PATH."
  (let ((stripped (replace-regexp-in-string "_build/default/" "" path nil t)))
    (if (file-exists-p stripped) stripped path)))
...
\[Locate Library Peano] in 8.4pl3)."
  (if (and (equal lib-path "")
    ...
    (let* ((lib-name (concat lib-path mod-name))
      ...
      (and path (company-coq--strip-dune-build-dir path)))))
