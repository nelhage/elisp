...
;; removed use-fonts check (due to incomatibilities) by Albert Cohen 9-00
...
(make-face 'sym-lock-adobe-symbol-face
	   "Face for Sym-Lock symbols")
(set-face-property 'sym-lock-adobe-symbol-face
...
(defun sym-lock-pre-idle-hook-first ()
  (if sym-lock-enabled
	  ...
	  (if (and sym-lock-enabled font-lock-old-extent)
	      (setq sym-lock-ext-start (extent-start-position
					font-lock-old-extent)
		    sym-lock-ext-end (extent-end-position
				      font-lock-old-extent))
...
(defun sym-lock-pre-idle-hook-last ()
  (if sym-lock-enabled
	  ...
	  (if (and sym-lock-enabled sym-lock-ext-start)
	      (sym-lock-make-symbols-atomic sym-lock-ext-start
					    sym-lock-ext-end))
